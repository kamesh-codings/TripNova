...
export const SafetyHub: React.FC<SafetyHubProps> = ({
  ...
  return (
    <div className="animate-fade" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      {/* SOS Modal Alert */}
      {isSOSModalOpen && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(12px)' }}>
          <div className="glass-panel animate-fade" style={{ width: '100%', maxWidth: '480px', padding: '24px', background: '#090e17', border: '2px solid #ef4444', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <div style={{ width: '56px', height: '56px', borderRadius: '50%', background: 'rgba(239, 68, 68, 0.2)', border: '2px solid #ef4444', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <AlertOctagon style={{ width: '28px', height: '28px', color: '#f87171' }} />
              </div>
              <h3 style={{ fontSize: '1.25rem', fontWeight: 900, color: '#ffffff' }}>EMERGENCY SOS BROADCAST</h3>
              <p style={{ fontSize: '0.78rem', color: '#cbd5e1' }}>
                Transmits your live GPS coordinates, battery level, and medical allergies to your 5 registered trusted contacts and nearest precinct.
              </p>
            </div>

            <div style={{ padding: '12px 14px', background: 'rgba(15, 23, 42, 0.9)', borderRadius: '14px', border: '1px solid rgba(255,255,255,0.08)', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <span style={{ fontSize: '0.72rem', fontWeight: 800, color: '#fca5a5', textTransform: 'uppercase' }}>5 Registered Recipients:</span>
              <div className="flex flex-wrap gap-1">
                {userProfile.trustedContacts.map((c, i) => (
                  <span key={i} style={{ fontSize: '0.7rem', padding: '3px 8px', borderRadius: '6px', background: '#1e293b', color: '#f8fafc' }}>
                    {c.name} ({c.phone})
                  </span>
                ))}
              </div>
              <div style={{ fontSize: '0.72rem', color: '#38bdf8', display: 'flex', alignItems: 'center', gap: '6px', paddingTop: '4px' }}>
                <Radio style={{ width: '14px', height: '14px' }} />
                Live GPS: 13.0827° N, 80.2707° E (Chennai Tourist Zone)
              </div>
            </div>

            {sosSent ? (
              <div style={{ padding: '16px', background: 'rgba(16, 185, 129, 0.2)', border: '1px solid #10b981', borderRadius: '14px', textAlign: 'center', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <CheckCircle style={{ width: '32px', height: '32px', color: '#34d399', margin: '0 auto' }} />
                <h4 style={{ fontSize: '0.9rem', fontWeight: 800, color: '#ffffff' }}>SOS Distress Alert Broadcasted!</h4>
                <p style={{ fontSize: '0.75rem', color: '#cbd5e1' }}>
                  SMS alerts & GPS tracking links have been dispatched to all 5 contacts.
                </p>
                <button
                  onClick={() => {
                    setSosSent(false);
                  ...
                  className="btn-primary"
                  style={{ marginTop: '8px', padding: '8px 16px', fontSize: '0.78rem' }}
                >
                  Close Radar
                </button>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                ...
                <div className="grid grid-2 gap-2">
                  <a
                    href="tel:112"
                    className="btn-secondary"
                    style={{ color: '#f87171', borderColor: 'rgba(239, 68, 68, 0.4)', justifyContent: 'center' }}
                  >
                    Direct Dial 112
                  </a>
                  <button
